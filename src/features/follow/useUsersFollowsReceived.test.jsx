...
describe("useUsersFollowsReceived", () => {
  let fetchMock;
  ...
  it("starts with no users or error while the request is pending", () => {
    ...
    const { result } = renderHook(() => useUsersFollowsReceived());

    expect(result.current.usersFollowsReceived).toBeNull();
    expect(result.current.error).toBeNull();
    expect(result.current.loading).toBe(true);
  });

  it("requests received follow requests with credentials and an abort signal", () => {
    fetchMock.mockReturnValue(new Promise(() => {}));

    renderHook(() => useUsersFollowsReceived());

    expect(fetchMock).toHaveBeenCalledWith(
      "https://api.example.com/v1/follow-requests/received",
      {
        method: "GET",
        credentials: "include",
  ...
  it("stores the users from the response and finishes loading after success", async () => {
    ...
    expect(result.current.usersFollowsReceived).toEqual(users);
    expect(result.current.error).toBeNull();
  ...
  it("aborts the pending received follow requests request when unmounted", () => {
    fetchMock.mockReturnValue(new Promise(() => {}));

    const { unmount } = renderHook(() => useUsersFollowsReceived());
