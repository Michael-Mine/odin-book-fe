...
describe("UnfollowButton", () => {
  ...
  function renderButton() {
    ...
    render(<UnfollowButton userCuid="requester-1" />);
    return user;
  }

  async function expectError() {
    expect(await screen.findByRole("heading", {
      name: "Unable to complete unfollow request",
    })).toBeInTheDocument();
    expect(screen.queryByRole("button")).not.toBeInTheDocument();
  }

  it("shows an enabled Unfollow button without fetching on mount", () => {
    renderButton();
  ...
  it("posts to the user's unfollow endpoint with credentials", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledExactlyOnceWith(
      "https://api.example.com/v1/follow-requests/requester-1/unfollow",
      { method: "POST", credentials: "include" },
    );
    expect(await screen.findByRole("button", { name: "Unfollowed" })).toBeInTheDocument();
    expect(screen.queryByRole("button", { name: "Unfollow", exact: true })).not.toBeInTheDocument();
  });
  ...
  it("prevents duplicate requests while fetching and parsing the response", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "Unfollow" }));
    const pendingButton = screen.getByRole("button", { name: "Unfollowing..." });
    ...
    expect(screen.getByRole("button", { name: "Unfollowed" })).toBeInTheDocument();
    expect(screen.queryByRole("button", { name: "Unfollowing..." })).not.toBeInTheDocument();
  });

  it("does not send another request when Unfollowed is clicked", async () => {
    fetchMock.mockResolvedValue({
      ok: true,
      json: () => Promise.resolve({ follow: { id: 1 } }),
    });
    const user = renderButton();
    await user.click(screen.getByRole("button", { name: "Unfollow" }));
    await user.click(await screen.findByRole("button", { name: "Unfollowed" }));
