...
describe("useUsersFollowsSent", () => {
  ...
  it("starts with no users or error while the request is pending", () => {
    ...
    const { result } = renderHook(() => useUsersFollowsSent());

    expect(result.current.usersFollowsSent).toBeNull();
    expect(result.current.error).toBeNull();
    expect(result.current.loading).toBe(true);
  });

  it("requests sent follow requests with credentials and an abort signal", () => {
    fetchMock.mockReturnValue(new Promise(() => {}));

    renderHook(() => useUsersFollowsSent());

    expect(fetchMock).toHaveBeenCalledWith(
      "https://api.example.com/v1/follow-requests/sent",
      {
        method: "GET",
        credentials: "include",
  ...
  it("stores the users from the response and finishes loading after success", async () => {
    ...
    expect(result.current.usersFollowsSent).toEqual(users);
    expect(result.current.error).toBeNull();
  ...
  it("aborts the pending request when unmounted", () => {
    ...
    const { unmount } = renderHook(() => useUsersFollowsSent());
